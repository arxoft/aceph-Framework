...
jQuery.fn.validationEngine = function(settings) {
	if($.validationEngineLanguage){					// IS THERE A LANGUAGE LOCALISATION ?
		...
		allRules = {"required":{    			  // Add your regex rules here, you can take telephone as an example
						...
						"telephone":{
							"regex":"/^[0-9]{3}\-[0-9]{3}\-[0-9]{4}( ext [0-9]+)?$/",
							"alertText":"* Invalid phone number<br />* Must be in the format (xxx-xxx-xxxx) or (xxx-xxx-xxxx ext xx"},
						"zipcode":{
							"regex":"/^[0-9\-\(\)\ ]{5}$/",
							"alertText":"* Invalid zipcode<br />* Must be 5 digits code"},								
						"email":{
						...
						"checkExistance":{
							"regex":"none",
							"alertText":"* Field value already exists, try other value"}
					}	
	}
	...
	var validateCall = function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FILED
		...
		for (i=0; i<rules.length;i++){
			...
			break;
			case "checkExistance": 
				 _checkExistance(caller,rules,i);
			break;
			default :;
			};
		};
		...
		function _checkExistance(caller,rules,position){		 // VALIDATE FIELD MATCH
			confirmField = rules[position+1]
			//var c = document.getElementsByName(confirmField);
			
			//for(i=0;i<c.lenth;i++){
				if($(caller).attr('value') == $("#"+confirmField).attr('value')){
					isError = true;
					promptText += settings.allrules["checkExistance"].alertText+"<br />";
					//break;
				}
			//}
		}
